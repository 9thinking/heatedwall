...
for LEVEL in 8; do
	for RADIUS in 0.00125; do
		for VELOCITY in 0.11; do

			cp -r MasterImpact/ ContactRadius-R$RADIUS-V$VELOCITY-Level$LEVEL
			cd ContactRadius-R$RADIUS-V$VELOCITY-Level$LEVEL/

			qcc -O2 -w -fopenmp -Wall DropImpact_Heat_Version2.c -lm -o DropImpact -L$BASILISK/gl -lglutils -lfb_tiny
			# parameters:
			# 1. rho liquid
			# 2. rho gas
			# 3. mu liquid
			...
			./DropImpact 1000 1.21 0.001 1.81e-5 0.0616 9.81 $RADIUS $VELOCITY 10.001 $LEVEL
